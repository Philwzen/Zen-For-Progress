...
Function AuditedField Returns log (pv-tablename as char , pv-fieldname as char):
   if index(pv-fieldname,"[") ne 0 then
      pv-fieldname = substring(pv-fieldname,1,index(pv-fieldname,'[') - 1).
...
output stream op to 're3clean.log'.

def var holdid as dec no-undo.
find last zen-auditdetail where 
   zen-auditdetail.auditdate ge 05/15/2008 no-error.
if avail zen-auditdetail then holdid = zen-auditdetail.zen-auditdetailtableid.

etime(true).
put stream op unformatted 'Processing Unuadited Tables' skip.

for each zen-auditdetail no-lock where
zen-auditdetail.zen-auditdetailtableid ge holdid:
    if x mod 100 = 0 then put screen string(zen-auditdetail.auditdate).
    if not AuditedTable(zen-auditdetail.tablename)
    then do: /* not supposed to be audited at all */
        put stream op 
           zen-auditdetail.auditdate " table: " zen-auditdetail.tablename skip.
        ...
        for each zen-auditline of zen-auditdetail no-lock:
            ...
            then do:
               put stream op 
                  zen-auditdetail.auditdate " field: " 
                  zen-auditdetail.tablename form "x(20)" " "
                  zen-auditline.fieldname   form "x(20)"
                  skip.
               delline(rowid(zen-auditline)).
            end.
         end. 
