...
/* If not can-find(FIRST Zen-Auditfield where zen-auditfield.zen-auditconfigtableid = Zen-AuditConfig.zen-auditconfigtableid
...
def var pv-value as char no-undo.
  find first zen-context where Zen-Context.server-connection-id = trim(SESSION:server-connection-context)
