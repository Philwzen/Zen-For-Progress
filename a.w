...
def var lv-page         as int    no-undo initial 0.
...
def var lv-delim        as char  no-undo initial " ".

def var lv-gotlast as log no-undo.
lv-delim = " ". /* really needs new field on lookup record */
...
def var lv-search as char FORMAT "X(256)":U 
     LABEL "Search" 
     VIEW-AS FILL-IN 
     SIZE 34.2 BY 1
      NO-UNDO.
...
DEFINE FRAME FRAME-ok
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 2 ROW 21 SCROLLABLE 
          WIDGET-ID 200.

DEFINE FRAME FRAME-select
     lv-search AT ROW 1.48 COL 28.8 COLON-ALIGNED WIDGET-ID 28
     btn-search AT ROW 1.48 COL 66 WIDGET-ID 6 NO-TAB-STOP 
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 82 BY 3.1
...
def var XXTABVALXX as log NO-UNDO.
...
def var x as int no-undo.
 ...
 /* this really needs modding to deal with multiple tables 
    ...
    it will fail. unlikely but possile ! */
...
no-error.
h-browse:select-row(1) no-error.
ReorderBrowseColumns(h-browse,lv-properorder).

end.
else do:
    h-buffer[1] = pv-dataset:GET-BUFFER-HANDLE(1).
    lv-exclude[1] = substring(h-buffer[1]:table,3) + 'tableid'.
    lv-width = h-browse:width.
    h-qry = h-browse:query.
    h-qry:SET-BUFFERS(h-buffer[1]).
    lv-where  = 'for each ' + h-buffer[1]:NAME + ' ' + pv-by.
    h-qry:QUERY-PREPARE(lv-where).
    h-qry:QUERY-OPEN().
    lv-repowhere = 'where ' + lv-exclude[1] + ' = ' + string(pv-id).
    h-buffer[1]:find-first(lv-repowhere) no-error.

message h-browse:query:num-results skip 
        numrecords('',h-buffer[1])
{&dbt}.
...
/* {&msb} 'prepare : ' h-qry:prepare-string skip */
/*        'Index   : ' h-qry:index-information skip */
/*        'Where   : ' lv-where skip */
/*        'Repo    : ' lv-repowhere skip */
/*        'page    : ' lv-page skip */
/*        'records : ' h-qry:num-results skip */
/*        'sqry    : ' lv-getwhere skip */
/*        'tableid : ' lv-exclude[1] + ' = ' + string(pv-id) */
/* {&dbt}. */
...
if t-zen-fldlook.tablename = 'acct' and lv-acctdefid = '' 
