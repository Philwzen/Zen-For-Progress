...
&glob about
{app-paths.i}
def input param pv-lookup as char  no-undo.

def var pv-id as dec  no-undo.

&glob table-name zen-fldlook

def var pv-startvalue As Char No-undo.
...
def var lv-key  as char no-undo init '**'.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD SetLookupId ialog-Frame 
FUNCTION SetLookupId RETURNS INTEGER
  (  pv-lookup as char )  FORWARD.
...
DEFINE FRAME FRAME-ok
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2 ROW 18.91 SCROLLABLE  WIDGET-ID 200.
...
def var lv-joins as char no-undo.

lv-joins = replace(GetNamedValue("relations",t-zen-fldlook.extra-details),';',',').
...
{{&core}run.i &program   = "dynamic.p"
              ...
              &params    = "(lv-page,
                             lv-tables,
                             lv-getwhere,
                             lv-joins,  /* dataset linking keyfields not used */
                             lv-returning,
...
if num-entries(t-zen-fldlook.tablename) > 1 
then
    lv-returning = entry(1,t-zen-fldlook.tablename) + 'tableid;' + 
                   entry(2,t-zen-fldlook.tablename) + 'tableid' + lv-returning.
else       
...
errorclear().
 {{&core}run.i
         &program   = "zen-fldlook.p"
         &path      = "{&core}{&srv}"
         &Appsrv    = "System"
         &procedure = "find-record"
         &params    = "(pv-id,
                        input-output table t-zen-fldlook)"}
if anyerrors() then return 'failed'.

find first t-zen-fldlook no-lock no-error.
...
do y = 1 to pv-dataset:num-buffers:
    ...
    do x = 1 to h-buf:num-fields:
/*     message x h-buf:buffer-field(x):name skip y lv-exclude[y] {&dbt}. */
...
def var lv-jtext as char no-undo.
...
if lv-searchField1 ne ''
then do:
if num-entries(t-zen-fldlook.tablename) = 1 then lv-jtext = ' and '.
                                            else lv-jtext = ' where ' .
    lv-where = lv-where + lv-jtext + lv-searchField1 + lv-eqexpr + '"' + lv-searchvalue1 + '"'.
    lv-getby = ' by ' + lv-searchField1. 
end.
if lv-searchField2 ne '' 
then do:
...
do x = num-entries(pv-fieldorder) to 1 by -1:
    ...
    h-browse:move-column(lv-from,lv-to) no-error.
end.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION SetLookupId ialog-Frame 
FUNCTION SetLookupId RETURNS INTEGER
  (  pv-lookup as char ) :
    def var pv-id as int no-undo.
    
    pv-id = int(pv-lookup) no-error.
    if error-status:error 
    then pv-id = int(getfield('zen-fldlook','lookupname',pv-lookup,'zen-fldlooktableid')).

  RETURN pv-id.   /* Function return value. */
