...
&IF DEFINED(EXCLUDE-get-colour) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-colour Procedure 
...
------------------------------------------------------------------------------*/
    ...
    def input param ip-colour-name         as char                         no-undo.
    def output param op-colour-value        as int          init ?          no-undo.


    find {&table-name} where {&table-name}.colour_name = ip-colour-name
         no-lock no-error.

    if available {&table-name} then
        assign op-colour-value = {&table-name}.colour_value.
...
&IF DEFINED(EXCLUDE-get-colour-rgb) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-colour-rgb Procedure 
...
------------------------------------------------------------------------------*/
    ...
    def input param ip-colour-name         as char                         no-undo.
    def output param op-colour-value        as char           init '?'      no-undo.
    ...
    if available {&table-name} then
    assign op-colour-value = string({&table-name}.red_value) + ',' +
                             string({&table-name}.green_value) + ',' +
                             string({&table-name}.blue_value).
...
------------------------------------------------------------------------------*/
    if can-find(first {&table-name} 
                where {&table-name}.colour_name = t-{&table-name}.colour_name) then
    do:
        ErrorCreate(122,"Colour ID",t-{&table-name}.colour_name,"","").
    end.
