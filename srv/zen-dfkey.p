...
def input  param pv-lastkey as char no-undo.
def output param pv-action  as char no-undo.


{{&core}find.i &table = "zen-dfkey"
              &where = "where zen-dfkey.fkey = pv-lastkey
                          And can-do(zen-dfkey.in-prog,pv-prog)"
              &type  = "first"            
              &lock  = "no"}
        
If Avail zen-dfkey Then
    pv-action = zen-dfkey.action.
Else Return 'Failed'.        
