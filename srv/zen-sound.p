...
def input param pv-code as char no-undo.
   ...
   {{&core}apsrv-findrecord.i 
        &where = "where Zen-Sound.EventName = pv-code"}
...
&IF DEFINED(EXCLUDE-GetSound) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE GetSound Procedure 
procedure GetSound :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
------------------------------------------------------------------------------*/
def input param pv-code as char no-undo.
def output param pv-soundfile as char no-undo.

find first zen-sound where Zen-Sound.EventName = pv-code
                     no-lock no-error.
  if avail zen-sound 
      then pv-soundfile = Zen-Sound.SoundFile.
      else pv-soundfile = ''.
end procedure.
...
def var lv-string as char no-undo.
lv-string = BuildCombo(?,
                      "zen-sound",
                      "eventname",
                      "soundfile",
                      "",
...
assign pv-codes = entry(1,lv-string,'{&Delim3}')
       pv-values = entry(2,lv-string,'{&Delim3}').
