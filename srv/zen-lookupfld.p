...
&glob table-name zen-lookupfld    /* name of table we are using */
...
&IF DEFINED(EXCLUDE-duplicate) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE duplicate Procedure 
procedure duplicate :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
def buffer b-table for {&table-name}. 

for each b-table where b-table.lookupname = pv-from
                        no-lock.

    create {&table-name}.
    buffer-copy b-table except lookupname {&table-name}tableid to {&table-name}.
    {&table-name}.lookupname = pv-to.
    release {&table-name}.
end.   
...
def input param pv-lkid as char no-undo.
   {{&core}apsrv-getrecords.i &whereclause = "where zen-lookupfld.lookupname = pv-lkid"}
...
{{&core}apsrv-openquery.i 
   &where = "where zen-lookupfld.lookupname = pv-lookupName"}
