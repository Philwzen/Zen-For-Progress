...
&glob table-name zen-log     /* name of table we are using */
...
&IF DEFINED(EXCLUDE-dellog) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE dellog Procedure 
procedure dellog :
def input param pv-type as char no-undo.
def input param pv-from as date no-undo.
def input param pv-to as date no-undo.
def input param pv-by as char no-undo.



for each zen-log where zen-log.logtype matches pv-type
            and zen-log.logdate >= pv-from
            and zen-log.logdate <= pv-to
            and zen-log.LogUser matches pv-by
            exclusive-lock:
       delete zen-log.
end.
...
&whereclause = "where zen-log.logtype matches pv-type
            and zen-log.logdate >= pv-from
            and zen-log.logdate <= pv-to
            and zen-log.LogUser matches pv-by"}
...
&IF DEFINED(EXCLUDE-MakeLog) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE MakeLog Procedure 
procedure MakeLog :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
...
if pv-logfile = '' then do:
   create zen-log.
   assign
       zen-log.logtype = pv-type
       zen-log.logdate = today
       zen-log.LogTime = time
       zen-log.LogUser = lv-user
       zen-log.program = pv-prog
       zen-log.action  = pv-action
       zen-log.LogMessage = pv-msg.
   release zen-log.
end.
else do:
    output stream log to value(pv-logfile) append. 

    put stream log unformatted 
            pv-type ',' string(today,'99/99/9999') ',' string(time,'hh:mm:ss') ',' 
            lv-user ',' pv-prog ',' pv-action ',' pv-msg skip.
...
&where = "where zen-log.logtype matches pv-type
            and zen-log.logdate >= pv-from
            and zen-log.logdate <= pv-to
            and zen-log.LogUser matches pv-by"}
