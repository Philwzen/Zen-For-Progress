...
&IF DEFINED(EXCLUDE-get-currency) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-currency Procedure 
...
------------------------------------------------------------------------------*/
    ...
    find zen-country where zen-country.country = ip-country
         no-lock no-error.

    assign op-currency = if available zen-country then
                                zen-country.default-currency
                         else
                                "".
...
assign lv-data = BuildCombo(?,
                            '{&table-name}',
                            'zen-country.country',
                            'zen-country.description',
                            '','by zen-country.description',no,no)
