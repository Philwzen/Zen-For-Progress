...
/* EXAMPLE
   ...
   */
    for each zen-widgetproperty where zen-widgetproperty.pgm = zen-dwidget.pgm 
                                  and zen-widgetproperty.widframe = zen-dwidget.widframe 
                                  and zen-widgetproperty.wid-name = zen-dwidget.wid-name
                                exclusive-lock:
        delete zen-widgetproperty.
    end.
end procedure.
...
def input param pv-pgm as char no-undo.
   ...
   {{&core}apsrv-getrecords.i &whereclause = "WHERE {&table-name}.pgm = pv-pgm"}
