...
&glob title-text Task Submit       /* message box title */
&glob suppresswindow
...
&Scoped-Define ENABLED-OBJECTS Btn-Cancel RECT-1 btn-Submit lv-program ~
lv-procedure cb-type lv-quick 
&Scoped-Define DISPLAYED-OBJECTS lv-program lv-procedure cb-type lv-quick 

/* Custom List Definitions                                              */
/* List-1,List-2,List-3,List-4,List-5,List-6                            */
&Scoped-define List-1 lv-program lv-procedure 
&Scoped-define List-2 lv-tserver 
...
def var lv-procedure as char FORMAT "X(256)":U 
     LABEL "Procedure" 
     VIEW-AS FILL-IN 
     SIZE 42.2 BY 1 NO-UNDO.

def var lv-program as char FORMAT "X(256)":U 
     LABEL "Program" 
     VIEW-AS FILL-IN 
     SIZE 42.2 BY 1 NO-UNDO.

def var lv-tserver as char FORMAT "X(256)":U 
     LABEL "Server" 
     VIEW-AS FILL-IN 
     SIZE 42.2 BY 1 NO-UNDO.
...
def var lv-quick as log INITIAL yes 
     LABEL "Quick Submit (one run only A.S.A.P)" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-main
     ...
     btn-Submit AT ROW 8.14 COL 22.4 HELP
          "Submit" WIDGET-ID 22
     lv-tserver AT ROW 2.43 COL 15.8 COLON-ALIGNED WIDGET-ID 14
     lv-program AT ROW 3.38 COL 15.8 COLON-ALIGNED WIDGET-ID 10
     lv-procedure AT ROW 4.33 COL 15.8 COLON-ALIGNED WIDGET-ID 8
     cb-type AT ROW 5.29 COL 15.8 COLON-ALIGNED WIDGET-ID 6
     lv-quick AT ROW 6.76 COL 12 WIDGET-ID 12
     "Adhoc Task Submit" VIEW-AS TEXT
          SIZE 24 BY .71 AT ROW 1.24 COL 3 WIDGET-ID 18
     RECT-1 AT ROW 1.71 COL 2 WIDGET-ID 16
...
/* SETTINGS FOR BUTTON btn-Schedule IN FRAME f-main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN lv-procedure IN FRAME f-main
   1                                                                    */
/* SETTINGS FOR FILL-IN lv-program IN FRAME f-main
   1                                                                    */
/* SETTINGS FOR FILL-IN lv-tserver IN FRAME f-main
   NO-DISPLAY NO-ENABLE 2                                               */
ASSIGN 
       lv-tserver:READ-ONLY IN FRAME f-main        = TRUE.
...
IF SEARCH(lv-program:SCREEN-VALUE) = ? THEN do:
   MESSAGE msg(50,'Program',lv-program:SCREEN-VALUE,'','') VIEW-AS ALERT-BOX ERROR.
   RETURN NO-APPLY.
END.
...
THEN DO:
   IF not lv-quick:checked
   then RUN submit-task(NO,lv-schedule).
   ELSE RUN submit-task(YES,'').
    /* task number #1 has been submitted to the taskserver #2 */   
    MESSAGE msg(173,string(lv-task-no),lv-tserver:SCREEN-VALUE,'','')
...
ON VALUE-CHANGED OF lv-quick IN FRAME f-main /* Quick Submit (one run only A.S.A.P) */
DO:
   assign lv-quick.
...
------------------------------------------------------------------------------*/
  DISPLAY lv-program lv-procedure cb-type lv-quick 
      WITH FRAME f-main.
  ENABLE Btn-Cancel RECT-1 btn-Submit lv-program lv-procedure cb-type lv-quick 
      WITH FRAME f-main.
...
*/
ASSIGN cb-type:LIST-ITEMS in frame {&frame-name} = '{&treport}{&combodelim}{&tutility}{&combodelim}{&tadhoc}'
       cb-type:PRIVATE-DATA = cb-type:LIST-ITEMS
       cb-type:SCREEN-VALUE = '{&tadhoc}'.
END PROCEDURE.
...
lv-tserver:screen-value in frame {&frame-name} = lv-server.
END PROCEDURE.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER lv-immidiate AS LOG NO-UNDO.
...
{{&core}run.i &program   = "zen-task.p"
             ...
             &noper     = true
             &procedure = "submit-task"
             &params    = "(lv-program:screen-value in frame {&frame-name},
                            lv-procedure:screen-value,  
                            lv-tserver:SCREEN-VALUE, 
                            IF lv-immidiate THEN TODAY ELSE lv-run-date,
                            IF lv-immidiate THEN TIME ELSE lv-run-time,
                            pv-sched, /* quick schedule */
                            cb-type:SCREEN-VALUE, /* task type */
                            '', /* parameters */
                            output lv-task-no)"} 
