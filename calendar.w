...
DEFINE VARIABLE lv-days AS INTEGER FORMAT "->>9":U INITIAL 0 
     VIEW-AS FILL-IN 
...
DEFINE FRAME Dialog-Frame
     lv-days AT ROW 13.29 COL 16.4 NO-LABEL WIDGET-ID 4
     rs-type AT ROW 12.43 COL 27.8 NO-LABEL WIDGET-ID 8
     Btn-go AT ROW 13.14 COL 46.4 WIDGET-ID 2
     "Today +" VIEW-AS TEXT
          SIZE 10 BY .86 AT ROW 13.33 COL 6 WIDGET-ID 12
     "Reposition To:" VIEW-AS TEXT
          SIZE 20.2 BY 1 AT ROW 11.62 COL 4.4 WIDGET-ID 16
     RECT-1 AT ROW 11.24 COL 2 WIDGET-ID 14
...
CREATE CONTROL-FRAME CtrlFrame ASSIGN
       ...
       HEIGHT          = 10.19
       WIDTH           = 48.2
       HELP            = "Double-click on any day number to select the date."
       HIDDEN          = no
       SENSITIVE       = yes.
...
on 'mouse-select-dblclick' anywhere bell.
on "MOUSE-MENU-CLICK":U    anywhere bell. 
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  if index(focus:format,'x') ne 0
then do:
   if length(focus:format) = 10
   then lv-str = string(ch-cal:value,'99/99/9999').
   else lv-str = string(ch-cal:value,'99/99/99').
end.
else lv-str = string(ch-cal:Value,focus:format).
...
assign
    ...
    ch-cal:Value = if pv-startvalue ne ' ' 
                   then date(pv-startvalue)
                   else today.
